...
const CreateEIvintation = () => {
	...
	const [load, setLoad] = useState({ status: '' })
	const { t, i18n } = useTranslation()
	const sendImage = (event) => {
		setimgUrl(URL.createObjectURL(event.target.files[0]))
		setLoad(prev => ({ ...prev, status: 'load' }))
		CreateElectronic(event.target.files, setNewCard, setLoad)
	}
	...
	return <main className={styles.create__card}>
		<form onSubmit={sendForm} name='create-card'>
			...
			<p style={{ color: '#fff', margin: '0px 0px 10px 0px' }}>{load.status === 'loaded' ? 'video uploaded' : ""}</p>
			{load.status === 'load' ? <div className={styles.create__electronicLoad}><img src='/5.gif' alt="" /></div> : ''}
